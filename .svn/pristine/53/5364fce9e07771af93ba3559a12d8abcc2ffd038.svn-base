...
public class TbUserController extends BaseController {
	...
	@PostMapping("/add")
	@ResponseBody
	public JSONObject add(TbUser entity,String roleIds,String orgId) {
		//获取登录用户信息
		TbUser user = getCurrentUser();
		if (null != user.getUserId()) {
			boolean seccess = tbUserService.save(user, roleIds, orgId,entity);
			if (seccess) {
	...
	public JSONObject getByUserName(String userName ) {
		...
		if (null != user.getUserId()) {
			if (null != userName) {
				...
				List list = tbUserService.findBySearch(u);
	...
	public JSONObject updateUser(TbUser entity ) {
		...
		if (null != user.getUserId()) {
			...
			if (null != tbUser) {
				tbUserService.updateNotNull(entity);
				return ResponseMessage.returnInfo(true,null,null,"save_success");
			}else {
	...
	@ResponseBody
	public JSONObject getByOrgId(TbUser tbUser, Page<TbUser> page) {
		TbUser user = getCurrentUser();
		if (null != user.getUserId()) {
			Page<TbUser> page1 = tbUserService.getByOrgId(tbUser, page);
			return ResponseMessage.returnInfo(true,page1,null,null);
		}else {
			return ResponseMessage.returnInfo(false, null, null, "error");
		}
	...
	public JSONObject getById(String userId) {
		...
		if (null != user.getUserId()) {
			if (null != userId) {
				List<TbUser> entity = tbUserService.getRoleIdAndOrgByUserId(userId);
