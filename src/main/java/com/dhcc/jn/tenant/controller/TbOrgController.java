...
public class TbOrgController extends BaseController {
	 ...
	 * @return
	 */
	@PostMapping("/getTbOrgTreeByzhId")
	@ResponseBody
	public JSONObject getTbOrgTreeByzhId(String zhId) {
		List<TbOrg> list = tbOrgService.getTbOrgTreeByzhId(zhId);
		if(null != list && !list.isEmpty()) {
			for(int i=0 ;i<list.size();i++) {
				TbOrg tbOrg = list.get(i);
				List<TbOrg> list1 = getTbOrgByParentOrgId(tbOrg.getOrgId());
	...
	public JSONObject findAll(String zhId ) {
		...
		if (null != user.getUserId()) {
			List<Map<String, Object>> list = tbOrgService.findOrgByzhId(zhId);
	...
	public JSONObject findOrgByzhId(String zhId) {
		...
		if (null != user.getUserId()) {
			if (null != zhId) {
				List<Map<String, Object>> list = tbOrgService.findByzhId(zhId);
				if (list != null && !list.isEmpty()) {
					return ResponseMessage.returnInfo(true, list, null, null);
				} else {
					return ResponseMessage.returnInfo(true, null, null, null);
	...
	@ResponseBody
	public JSONObject getByOrgNameAndOrgType(String zhId,String orgName,String orgType) {
		TbUser user = getCurrentUser();
		if (null != user.getUserId()) {
			List<TbOrg> list = tbOrgService.getByOrgNameAndOrgType(zhId,orgName,orgType);
			if (null != list && !list.isEmpty()) {
				return ResponseMessage.returnInfo(true,list,null,null);
			} else {
				return ResponseMessage.returnInfo(true,null,null,null);
	...
	public JSONObject getByOrgId(String orgId) {
		...
		if (null != user.getUserId()) {
			if (null != orgId) {
				...
				return ResponseMessage.returnInfo(true,org,null,null);
			}else {
				return ResponseMessage.returnInfoSpecial(false,null,null,"请输入组织ID");
			}
		}else {
			return ResponseMessage.returnInfo(false,null,null,"error");
		}
	...
	public JSONObject update(TbOrg entity ) {
		...
		if (null != user.getUserId()) {
			...
			if (null != tbOrg) {
				...
				tbOrgService.updateNotNull(entity);
				return ResponseMessage.returnInfo(true,null,null,"save_success");
			}else{
				return ResponseMessage.returnInfo(false,null,null,"save_fail");
			}
