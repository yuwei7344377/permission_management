...
public class TbTenantUserController extends BaseController {
	...
	@PostMapping("/add")
	@ResponseBody
	public JSONObject add(TbTenantUser entity,String userName,String passWord,String phone) {
		...
		if (null != user.getUserId()) {
			if (null != entity.getZhName()) {
				...
				if (null == tbTenantUser) {
					boolean seccess = tbTenantUserService.intsert(user, entity,  userName, passWord, phone);
					if (seccess) {
	...
	public JSONObject getzhIdByTenantAndUser(String zhId) {
		...
		if (null != user.getUserId()) {
			if (null != zhId) {
				List<TbTenantUser> list = tbTenantUserService.getzhIdByTenantAndUser(zhId);
